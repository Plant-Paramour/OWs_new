...
# [COUNCIL] Enabled depth-2 search 鈥� it was fully implemented but disabled.
...
# [COUNCIL] Enabled multiprong 鈥� fully implemented, just never turned on.
...
# [COUNCIL] Enabled 鈥� this feature was fully written (gated to 2P only) but
...
# This function is O(T脳P) per fleet and was called from World.__init__,
# forward_project, and _build_multiprong_attack without any memoization.
# Cache by (fleet_id, step, planet_count) 鈥� invalidated each turn automatically
# since step advances. On boards with 80 fleets 脳 20 orbital planets this
...
def _detect_mode(world):
    ...
    - streak < TWO_P_PATIENT_NUDGE_TURNS  鈫� use intended mode
    - streak >= TWO_P_PATIENT_NUDGE_TURNS  鈫� escalate to opportunistic
    - streak >= TWO_P_PATIENT_ESCALATE_TURNS 鈫� escalate to pressure
...
def _handle_search_expand_4p(world, available, spent, target_locked, moves, mode_log,
    ...
    for act in actions[:SEARCH_MAX_ACTIONS_TO_PICK * 2]:
        ...
        if (
                world.stop_expanding_2p or world.prod_lead_stop_expand_4p or world.turn_cutoff_stop_expand) and tgt is not None and tgt.owner == -1:
            continue
    ...
    def _commitment_viable(c):
        ...
        if FAILTOLERANT_ENABLED:
            ...
            if owner_at_commit is not None:
                ...
                if current_owner == -1:
                    return False  # neutral 鈥� commitment type changed
                if current_owner == world.player:
                    return False  # we already own it 鈥� no need to attack
                # Enemy 鈫� different enemy: commitment still valid
        return True
