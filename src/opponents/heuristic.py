"""启发式战术对手 —— 基于物理预测和战术评估的完整智能体。

来源: other's_work/orbit-wars-2026-tactical-heuristic.ipynb
包含行星旋转预测、彗星轨迹预测、舰队追踪、战斗模拟、太阳绕行路由等功能。
"""
...
def _segment_hits_sun(x1, y1, x2, y2, safety=SUN_SAFETY):
    ...
    a = dx * dx + dy * dy
    if a < 1e-9:
        return _dist(x1, y1, CENTER_X, CENTER_Y) < r
    b = 2 * (fx * dx + fy * dy)
    c = fx * fx + fy * fy - r * r
    disc = b * b - 4 * a * c
    if disc < 0:
...
def _safe_angle_and_distance(sx, sy, tx, ty):
    direct = _dist(sx, sy, tx, ty)
    if not _segment_hits_sun(sx, sy, tx, ty):
    ...
    for sign in (1.0, -1.0):
        for mult in (1.8, 2.3, 3.0, 4.0):
            ...
            if (_segment_hits_sun(sx, sy, wx, wy, safety=SUN_SAFETY)
                    or _segment_hits_sun(wx, wy, tx, ty, safety=SUN_SAFETY)):
                continue
...
def _predict_planet_position(planet, initial_by_id, angular_velocity, turns):
    init = initial_by_id.get(planet.id)
    if init is None:
        return planet.x, planet.y
    orbital_r = _dist(init.x, init.y, CENTER_X, CENTER_Y)
...
def _predict_comet_position(planet_id, comets, turns):
    for g in comets:
...
def _travel_time(src_x, src_y, tgt_x, tgt_y, ships):
    _, d = _safe_angle_and_distance(src_x, src_y, tgt_x, tgt_y)
...
def _aim_with_prediction(src, target, ships, initial_by_id, ang_vel, comets, comet_ids):
    ...
    for _ in range(5):
        angle, d = _safe_angle_and_distance(src.x, src.y, tx, ty)
        turns = max(1, int(math.ceil(d / _fleet_speed(max(1, ships)))))
        if target.id in comet_ids:
            pos = _predict_comet_position(target.id, comets, turns)
            if pos is None:
                return None
            ntx, nty = pos
        else:
            ntx, nty = _predict_planet_position(target, initial_by_id, ang_vel, turns)
...
def _simulate_planet_future(planet, arrivals, player_self, horizon):
    if not arrivals:
        if planet.owner == -1:
...
def _net_defense_needed(planet, arrivals, player):
    if not arrivals or planet.owner != player:
        return 0
    events = sorted(arrivals, key=lambda a: a[0])
    garrison = planet.ships
    last_t = 0
...
def _safe_ships_available(planet, arrivals, player, horizon):
...
def _nearest_distance_to_set(px, py, planets_set):
    if not planets_set:
        return 1e9
    return min(_dist(px, py, p.x, p.y) for p in planets_set)


def _relative_reaction_time(tgt, me_planets, enemy_planets):
    my_time = min((_travel_time(p.x, p.y, tgt.x, tgt.y, p.ships or 1)
                    for p in me_planets), default=1e9)
    enemy_time = min((_travel_time(p.x, p.y, tgt.x, tgt.y, p.ships or 1)
                       for p in enemy_planets), default=1e9)
    return my_time, enemy_time


def _indirect_wealth(planet, all_planets, player):
    w = 0.0
    for p in all_planets:
        if p.id == planet.id:
            continue
        d = _dist(planet.x, planet.y, p.x, p.y)
        if d < 1:
            continue
...
class HeuristicOpponent:
    ...
    包含完整的物理预测、舰队追踪、战斗模拟、太阳绕行路由。
    策略按阶段调整（早期扩张 → 中期压制 → 末期收割）。
    """
    ...
    def __call__(self, observation, configuration):
        ...
        if isinstance(observation, dict):
            ...
            get = lambda k, d=None: getattr(observation, k, d)
        ...
        my_total = (sum(p.ships for p in my_planets)
                    + sum(int(f.ships) for f in fleets if f.owner == player))
        ...
        for f in fleets:
            tgt, eta = _fleet_target_planet(f, planets)
            if tgt is None:
                continue
        ...
        reserve = {}
        for p in my_planets:
            need = _net_defense_needed(p, arrivals_by_planet[p.id], player)
            reserve[p.id] = min(p.ships, need)

        safe_available = {}
        for p in my_planets:
            safe_available[p.id] = _safe_ships_available(
                p, arrivals_by_planet[p.id], player, HORIZON)

        doomed_planets = set()
        for p in my_planets:
            timeline = _simulate_planet_future(p, arrivals_by_planet[p.id], player, HORIZON)
        ...
        for p in my_planets:
            if p.id in doomed_planets:
                ...
                available[p.id] = min(safe_available[p.id],
                                      max(0, p.ships - reserve[p.id]))
        ...
        for f in fleets:
            ...
            if f.owner == player:
                ...
                if (tgt.id not in inbound_friendly_eta
                        or inbound_friendly_eta[tgt.id] > eta):
        ...
        def compute_defender(tgt, arrival_turns):
            if tgt.owner == -1:
                ...
                enemy_before = sum(s for t, o, s in arrs
                                   if o != player and t <= arrival_turns)
                friend_before = sum(s for t, o, s in arrs
                                    if o == player and t < arrival_turns)
                defender = max(0, tgt.ships + tgt.production * arrival_turns
                               + friend_before - enemy_before)
            else:
                arrs = arrivals_by_planet.get(tgt.id, [])
                enemy_reinf = sum(s for t, o, s in arrs
                                  if o == tgt.owner and t <= arrival_turns)
                friend_before = sum(s for t, o, s in arrs
                                    if o == player and t < arrival_turns)
                defender = (tgt.ships + tgt.production * arrival_turns
                            + enemy_reinf - friend_before)
            return max(0, defender)

        def is_safe_neutral(tgt):
            if tgt.owner != -1:
                return False
            my_t, en_t = _relative_reaction_time(tgt, my_planets, enemy_planets)
            return my_t <= en_t - 2
        ...
        def target_value(tgt, arrival_turns, cost):
            ...
            if tgt.id in comet_ids:
                life = _comet_remaining_life(tgt.id, comets)
                turns_profit = max(0, min(turns_profit, life - arrival_turns))
                if turns_profit <= 0:
                    return -1
            base = prod * turns_profit
            iw = _indirect_wealth(tgt, planets, player)
        ...
        for src in my_planets:
            ...
            for tgt in planets:
                ...
                if d0 > 145:
                    continue
                est_turns = _travel_time(src.x, src.y, tgt.x, tgt.y,
                                         max(10, src_ships))
                if is_very_late and est_turns > remaining_steps - 3:
                    continue
                if tgt.id in comet_ids and est_turns >= _comet_remaining_life(tgt.id, comets):
                    continue
                ...
                aim = _aim_with_prediction(src, tgt, est_needed,
                ...
                if _segment_hits_sun(src.x, src.y,
        ...
        for score, sid, tid, angle, ships, turns, value in candidates:
            ...
            base_needed = compute_defender(planet_by_id[tid], turns) + 1
        ...
        if not is_very_late:
            for src in my_planets:
                ...
                for tgt in planets:
                    ...
                    committed = (target_commitments.get(tgt.id, 0)
                                 + inbound_friendly.get(tgt.id, 0))
                ...
                aim = _aim_with_prediction(src, tgt, send,
                                           initial_by_id, ang_vel, comets, comet_ids)
                if aim is None:
                    continue
                angle, turns, _, _ = aim
        ...
        for sid in doomed_planets:
            ...
            for tgt in enemy_planets:
                ...
                if d0 > 100:
                    continue
                turns_est = _travel_time(src.x, src.y, tgt.x, tgt.y, available[sid])
            ...
            if best is None:
                continue
            _, tgt, _ = best
            aim = _aim_with_prediction(src, tgt, available[sid],
                                       initial_by_id, ang_vel, comets, comet_ids)
            if aim is None:
                continue
            angle, turns, _, _ = aim
        ...
        if not is_very_late and (enemy_planets or neutral_planets) and len(my_planets) > 1:
            ...
            front_dist = {p.id: _nearest_distance_to_set(p.x, p.y, ref_set)
                          for p in my_planets}
            ...
            for r in sorted_by_rear:
                ...
                if mid_candidates:
                    mid_candidates.sort(key=lambda p: _dist(r.x, r.y, p.x, p.y))
                ...
                if send < 10:
                    continue
                aim = _aim_with_prediction(r, forward_target, send,
                                           initial_by_id, ang_vel, comets, comet_ids)
                if aim is None:
                    continue
                angle, turns, _, _ = aim
        ...
        if finishing and enemy_planets:
            ...
            for src in my_planets:
                ...
                for tgt in weak_enemy[:3]:
                    ...
                    if d0 > 120:
                        continue
                    turns_est = _travel_time(src.x, src.y, tgt.x, tgt.y, available[src.id])
